...
function buildAssistantSystemPrompt() {
  return [
    ...
    "你不能直接判断谁是最终责任人、谁最可疑，也不能给出最终真相；你要先回答玩家当前这句话本身。",
    ...
    "如果本轮提到调取证据影像、监控画面、截图、照片或图片附件，要在正文里自然说明影像正在整理或已经附上；不要让图片无声出现。",
  ].join("\n");
}
...
function buildRoutePrompt({
  ...
  return [
    ...
    "",
    "判定规则：",
    "- 如果本地解析意图是 SUBMIT_DEDUCTION，返回 case_assistant；这是玩家主动提交最终推理，不属于剧透请求。",
    "- 如果玩家明确点名某个可问询人物，并且是在问/审/追问这个人，返回 start_interrogation，并给 suspectId。",
    "- 如果当前已经在 interrogation 模式，且玩家用了“你/你们/你和/你当时/你为什么/你几点/你认识/你和当事人什么关系”等第二人称追问，默认返回 suspect_question，suspectId 用当前问询对象。",
    ...
    "- 如果玩家要求直接公布最终责任人、真相或最终答案，且本地解析意图不是 SUBMIT_DEDUCTION，返回 spoiler_request。",
    "",
    "JSON 格式：",
...
function buildUserPrompt({
  ...
  return [
    ...
    "影像提示：如果本轮新增证据、打开证据，或玩家要求看图片/照片/截图/监控画面，正文里要顺口说一句“我把对应证据影像也调出来/正在整理，稍等会贴上来”一类的话；不要夸大为已经证明结论。",
